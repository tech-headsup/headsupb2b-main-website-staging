...
// Note: mailer-service template-only path does not carry the uploaded PDF file.
// Only fileName + note are emailed. If/when mailer-service supports attachments,
// this handler will need a multipart variant.

const LOG = "sendUploadQuoteMail";
...
export default async function handler(req, res) {
  ...
  if (!checkEnv(res, ["MAILER_UPLOAD_QUOTE_TEMPLATE_ID"])) return;
  ...
  const subjectTemplate =
    process.env.MAILER_UPLOAD_QUOTE_SUBJECT ||
    "[HeadsupB2B] Quote Upload – {{name}} | {{contactNo}}";
  const subject = interpolate(subjectTemplate, { name, contactNo });

  try {
    const json = await sendMailerEmail({
      templateId: process.env.MAILER_UPLOAD_QUOTE_TEMPLATE_ID,
      to,
      data: {
        name,
        contactNo,
